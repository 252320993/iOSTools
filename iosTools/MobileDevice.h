...
#define AMSVC_SYSTEM_PROFILER       CFSTR("com.apple.mobile.system_profiler")
	...
	struct am_device_notification_callback_info {
		...
		uint32_t msg;           /* 4    one of ADNCI_MSG_* */
	...
	struct am_recovery_device {
		uint8_t unknown0[8];                              /* 0 */
		am_restore_device_notification_callback callback; /* 8 */
		void *user_info;                                  /* 12 */
		uint8_t unknown1[12];                             /* 16 */
		uint32_t readwrite_pipe;                          /* 28 */
		uint8_t read_pipe;                                /* 32 */
		uint8_t write_ctrl_pipe;                          /* 33 */
		uint8_t read_unknown_pipe;                        /* 34 */
		uint8_t write_file_pipe;                          /* 35 */
		uint8_t write_input_pipe;                         /* 36 */
	} __PACK;
	...
	struct am_device {
		uint8_t unknown0[16];   /* 0 - zero */
		uint32_t device_id;     /* 16 */
		uint32_t product_id;    /* 20 - set to AMD_IPHONE_PRODUCT_ID */
		int8_t *serial;         /* 24 - set to UDID, Unique Device Identifier */
		uint32_t unknown1;      /* 28 */
		uint32_t unknown2;      /* 32 - reference counter, increased by AMDeviceRetain, decreased by AMDeviceRelease*/
		uint32_t lockdown_conn; /* 36 */
		uint8_t unknown3[8];    /* 40 */
#if (__ITUNES_VER > 740)
		uint32_t unknown4;      /* 48 - used to store CriticalSection Handle*/
#endif
#if (__ITUNES_VER >= 800)
		uint8_t unknown5[24];  /* 52 */
#endif
	} __PACK;
	...
	__DLLIMPORT mach_error_t AMDeviceNotificationSubscribe(am_device_notification_callback callback, 
                                                           ...
                                                           void* cookie, /*unsigned int cookie,*/ 
                                                           struct am_device_notification **subscription);
     ...
     *      MDERR_DICT_NOT_LOADED   if the load_dict() call failed
     */
   __DLLIMPORT mach_error_t AMDeviceLookupApplications(struct am_device *device, CFStringRef apptype, CFDictionaryRef *result);
	...
	__DLLIMPORT mach_error_t AMDeviceSetValue(struct am_device *device, CFStringRef domain, CFStringRef cfstring, CFStringRef cfnewstring);
...
//                                                  service_name, int *socket_fd);
    ...
    mach_error_t AMDeviceStartService(struct am_device *device, CFStringRef service_name, struct afc_connection **handle, unsigned int *
                                      unknown);
    
    mach_error_t AMDeviceSecureStartService(struct am_device *device, CFStringRef service_name, unsigned int* unknown, struct afc_connection **conn);
    
    int AMDServiceConnectionGetSocket(struct afc_connection* conn);
	 ...
	 *      MDERR_AFC_OUT_OF_MEMORY if malloc() failed
     __DLLIMPORT afc_error_t AFCConnectionOpen(struct afc_connection *handle, unsigned int io_timeout,
     struct afc_connection **conn);
	 */
	__DLLIMPORT afc_error_t AFCConnectionOpen(SOCKET socket, unsigned int io_timeout,
                                              struct afc_connection **conn);
	...
	typedef void (*NOTIFY_CALLBACK)(CFStringRef notification, USERDATA data);
	//__DLLIMPORT mach_error_t AMDPostNotification(SOCKET socket, CFStringRef  notification, CFStringRef userinfo);
	__DLLIMPORT mach_error_t AMDObserveNotification(SOCKET socket, CFStringRef notification);
    ...
    /* edits by annidy */
    __DLLIMPORT mach_error_t AMDPostNotification(struct afc_connection* socket, CFStringRef  notification, CFStringRef userinfo);
	__DLLIMPORT mach_error_t AMDShutdownNotificationProxy(struct afc_connection* socket);
